...
export class SecretSelectionModal extends Modal {
    ...
    async onOpen() {
        ...
        // Header
        const header = contentEl.createEl("h2", { text: t("modals.secretSelection.title") });
        ...
        const searchInput = searchContainer.createEl("input", {
            type: "text",
            placeholder: t("modals.secretSelection.searchPlaceholder")
        });
        ...
        const addBtn = leftFooter.createEl("button", { text: t("modals.secretSelection.addSecret") });
        addBtn.onclick = () => {
        ...
        const cancelBtn = rightFooter.createEl("button", { text: t("common.cancel") });
        cancelBtn.onclick = () => this.close();

        const saveBtn = rightFooter.createEl("button", { text: t("common.save"), cls: "secret-save-btn mod-cta" });
        saveBtn.onclick = () => {
            if (this.selectedKey) {
                const secretStorage = (this.app as any).secretStorage;
                ...
                new Notice(t("modals.secretSelection.selectFirst"));
            }
        };
    ...
    renderList() {
        ...
        if (filtered.length === 0) {
            this.listContainer.createEl("div", { text: t("modals.secretSelection.notFound"), cls: "secret-list-empty" });
            return;
        }
        ...
        filtered.forEach(key => {
            ...
            if (isSelected) {
                mainClickArea.createSpan({ text: t("modals.secretSelection.selected"), cls: "secret-item-badge" });
            }
            ...
            if (isVisible) {
                ...
                valSpan.textContent = t("modals.secretSelection.masked");
            }
